...
class LoggerIterableObject:
    def __init__(self, storage: Storage):
        self._storage = storage

    def __iter__(self):
        return LoggerIterator(self)

    def get_by_id(self, idx: int) -> LogContent:
        return self._storage.get_by_id(idx)
...
class LoggerIterator:
    def __init__(self, logger: LoggerIterableObject):
        self._logger = logger
        self._iter_idx = None
    ...
    def __next__(self):
        ...
        try:
            cur_log = self._logger.get_by_id(self._iter_idx)
        except IndexError:
            raise StopIteration
