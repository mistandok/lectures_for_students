...
def calculation(logger: Logger) -> int:
    result = 1 + 1
    logger.info(f"calculation result is {result}")
    return result


def foo_with_error(logger: Logger):
    try:
        _ = 10 / 0
    except ZeroDivisionError:
        logger.error(f"error in {foo_with_error.__name__}", with_tb=True)
